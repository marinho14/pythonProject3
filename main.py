...
if __name__ == '__main__':
    ...
    imagen_ILL = lista[-1][-1]  ## Se usa la ultima imagne de la lista ILL
    img_int=mt.Interpolacion(4,imagen_ILL)  ## Se interpola la ultima imagen de "lista"
    # Se usa un for para imprimir las imagenes pedidas de "lista"
    num = 1
    for a in (lista):
        for b in a:
            cv2.imshow("Ciclo" + str(num), b)
            num += 1
    ##Se imprimen las imagenes pedidas, la original y la interpolada
